...
def draw_text(I,l,cat,color=(255,0,0),thickness=1):
	wh = np.array(I.shape[1::-1]).astype(float)    
	tl = tuple((l.tl()*wh).astype(int).tolist())
	cv2.putText(I, str(cat), tl, cv2.FONT_HERSHEY_COMPLEX, 1, color)    
    
def draw_text2(I,l,cat,color=(255,0,0),thickness=1):
	wh = np.array(I.shape[1::-1]).astype(float)   
	tl = (l.tl()*wh).astype(int).tolist()
	cen = (l.tl() + l.br())/2.
	cen = (cen*wh).astype(int).tolist()
	cen = tuple((tl[0], cen[1]))
	cv2.putText(I, str(cat), cen, cv2.FONT_HERSHEY_COMPLEX, 1, color)  
